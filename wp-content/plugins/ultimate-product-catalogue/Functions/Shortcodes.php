...
function BuildSidebar($category, $subcategory, $tags, $prod_name) {
	...
	if ( $ProdCatString != "" )
	{
		$ProdCatString = esc_sql( $ProdCatString );
		
		$Categories = $wpdb->get_results("
			SELECT Category_ID, Category_Name, Category_Image 
				FROM {$categories_table_name} 
				WHERE Category_ID IN ({$ProdCatString})
				ORDER BY Category_Sidebar_Order
		");
	...
	if ( $ProdSubCatString != "" )
	{
		$ProdSubCatString = esc_sql( $ProdSubCatString );
		
		$SubCategories = $wpdb->get_results("
			SELECT SubCategory_ID, SubCategory_Name, SubCategory_Image, Category_ID 
				FROM {$subcategories_table_name} 
				WHERE SubCategory_ID IN ({$ProdSubCatString}) 
				ORDER BY SubCategory_Sidebar_Order
		");
	...
	if ( $ProdTagString != "" )
	{
		$ProdTagString = esc_sql( $ProdTagString );

		$Tags = $wpdb->get_results("
			SELECT Tag_ID, Tag_Name 
				FROM {$tags_table_name} 
				WHERE Tag_ID IN ({$ProdTagString}) 
				ORDER BY Tag_Sidebar_Order
		");
	...
	{
		$ProdCustomFieldsString = esc_sql( $ProdCustomFieldsString );
		
		$Custom_Fields = $wpdb->get_results("
			SELECT Field_ID, Field_Name, Field_Control_Type 
				FROM {$fields_table_name} 
				WHERE Field_ID IN ({$ProdCustomFieldsString}) AND Field_Searchable = 'Yes' 
				ORDER BY Field_Sidebar_Order
		");
...
function UPCP_Single_Page_Inquiry_Form() {
	...
	if ($CF_7_Installed or $WP_Forms_Installed) {
		...
		$Product_Inquiry_Please_Use_Label = empty( $Product_Inquiry_Please_Use_Label ) 
			? __("Please use the form below to enquire about the following products:", 'ultimate-product-catalogue')
			: $Product_Inquiry_Please_Use_Label;
		$ReturnString .= $Product_Inquiry_Please_Use_Label . "<br>";
		if (isset($_COOKIE['upcp_cart_products'])) {
			$Products_Array = explode(",", $_COOKIE['upcp_cart_products']);
